...
def test_absorption_line(answer_store):
    ...
    spectrum_answer_testing(spec, "absoprtion_line_test.h5", answer_store)

    pt_src_pos = PointSourceModel(30.0, 45.0)
    sim_cat = SimputCatalog.from_models("absorption_line", "absorption_line",
                                        spec, pt_src_pos, exp_time, area, prng=prng)
    sim_cat.write_catalog(overwrite=True)

    instrument_simulator("absorption_line_simput.fits", "absorption_line_evt.fits",
                         exp_time, inst_name, [30.0, 45.0], instr_bkgnd=False,
                         ptsrc_bkgnd=False, foreground=False, prng=prng)

    file_answer_testing("EVENTS", "absorption_line_evt.fits", answer_store)

    write_spectrum("absorption_line_evt.fits", "absorption_line_evt.pha",
                   overwrite=True)
