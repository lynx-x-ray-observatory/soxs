...
def make_background_events(out_file, exp_time, instrument, sky_center, clobber=False, 
                           roll_angle=0.0, astro_bkgnd=True, instr_bkgnd=True, 
                           prng=np.random):
    """
    Generate an events file comprised entirely of background photons. 

    Parameters
    ----------
    out_file : string
        The name of the background event file to be written.
    exp_time : float
        The exposure time to use, in seconds. 
    instrument : string
        The name of the instrument to use, which picks an instrument
    ...
    Examples
    --------
    >>> make_background_events("bkg_evt.fits", 30000.0, "hdxi_3x10", [30., 45.], clobber=True)
    """
    instrument_simulator(None, out_file, exp_time, instrument, sky_center, clobber=clobber,
                         roll_angle=roll_angle, astro_bkgnd=astro_bkgnd, instr_bkgnd=instr_bkgnd,
                         prng=prng)
