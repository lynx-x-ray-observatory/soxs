...
def combine_event_files(in_file1, in_file2, out_file, clobber=False):
    ...
    tbhdu_gti = f1["STDGTI"].copy()

    exp_time1 = hdu1.header["EXPOSURE"]
    exp_time2 = hdu2.header["EXPOSURE"]

    if exp_time1 > exp_time2:
        raise RuntimeError("The second file does not have sufficient exposure to "
                           "add its events to the first file! exp_time %g ")
    idxs = hdu2.data["TIME"] < exp_time2
    ...
    if hdu2.header["ROLL_PNT"] == hdu2.header["ROLL_PNT"]:
        pix = np.array([hdu2.data["X"], hdu2.data["Y"]])
    else:
        roll_angle = np.deg2rad(hdu1.header["ROLL_PNT"])
        rot_mat = np.array([[np.sin(roll_angle), -np.cos(roll_angle)],
                            [-np.cos(roll_angle), -np.sin(roll_angle)]])
        pix = np.dot(rot_mat, np.array([hdu2.data["DETX"], hdu2.data["DETY"]]))
        pix[0,:] += hdu1.header["TCRPX2"]
        pix[1,:] += hdu1.header["TCRPX3"]
    col_x = pyfits.Column(name='X', format='D', unit='pixel', 
                          array=np.concatenate([hdu1.data["X"], pix[0,:]]))
    col_y = pyfits.Column(name='Y', format='D', unit='pixel', 
                          array=np.concatenate([hdu1.data["Y"], pix[1,:]]))
    col_cx = pyfits.Column(name='CHIPX', format='D', unit='pixel',
                           array=np.concatenate([hdu1.data["CHIPX"],
                                                 hdu2.data["CHIPX"][idxs]]))
    col_cy = pyfits.Column(name='CHIPY', format='D', unit='pixel',
                           array=np.concatenate([hdu1.data["CHIPY"],
                                                 hdu2.data["CHIPY"][idxs]]))
    col_dx = pyfits.Column(name='DETX', format='D', unit='pixel',
                           array=np.concatenate([hdu1.data["DETX"],
                                                 hdu2.data["DETX"][idxs]]))
    col_dy = pyfits.Column(name='DETY', format='D', unit='pixel',
                           array=np.concatenate([hdu1.data["DETY"],
                                                 hdu2.data["DETY"][idxs]]))
    col_e = pyfits.Column(name="ENERGY", format="E", unit="eV", 
                          array=np.concatenate([hdu1.data["ENERGY"],
                                                hdu2.data["ENERGY"][idxs]]))
    col_ch = pyfits.Column(name=chantype, format='1J', unit=cunit,
                           array=np.concatenate([hdu1.data[chantype],
                                                 hdu2.data[chantype][idxs]]))
